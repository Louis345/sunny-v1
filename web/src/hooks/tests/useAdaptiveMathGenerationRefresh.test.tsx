...
describe("useAdaptiveMathGenerationRefresh", () => {
  ...
  it("does not silently ignore a human progress check while the automatic request is still running",async()=>{
    let automaticRequestAborted=false;
    const onStatusChanged=vi.fn();
    const fetchMock=vi.fn()
      .mockImplementationOnce((_url,options)=>new Promise((_resolve,reject)=>options?.signal?.addEventListener("abort",()=>{
        automaticRequestAborted=true;
        reject(new Error("manual_refresh_replaced_automatic_request"));
      },{once:true})))
      .mockResolvedValueOnce({ok:true,json:async()=>({updatedAt:"manual",phase:"board_ready",nodes:[]})});
    vi.stubGlobal("fetch",fetchMock);
    const {result}=renderHook(()=>useAdaptiveMathGenerationRefresh({childId:"ila",homeworkId:"hw-1",enabled:true,onStatusChanged}));
    await act(async()=>{await Promise.resolve();});
    expect(fetchMock).toHaveBeenCalledTimes(1);
    await act(async()=>{result.current.checkNow();await Promise.resolve();});
