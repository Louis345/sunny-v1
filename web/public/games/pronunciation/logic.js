...
export function buildNodeResult({
  ...
  const hitEvents = Math.max(0, Number(wordsHit) || 0);
  const uniqueTargets = Math.max(0, Number(wordsAttempted) || 0);
  const boundedWordsHit = Math.min(hitEvents, uniqueTargets);
  const accuracy = uniqueTargets > 0 ? boundedWordsHit / uniqueTargets : 0;
  const rounds = uniqueTargets > 0 ? Math.max(1, Math.ceil(hitEvents / uniqueTargets)) : 0;
  ...
  return {
    ...
    accuracy,
    timeSpent_ms,
    wordsHit: boundedWordsHit,
    wordsAttempted: uniqueTargets,
    hitEvents,
    uniqueTargets,
    rounds,
    flaggedWords,
