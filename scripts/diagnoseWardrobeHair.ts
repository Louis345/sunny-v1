...
const results=[];
for(const [primitiveIndex,p] of asset.json.meshes[2].primitives.entries()){
...
writeFileSync('artifacts/wardrobe-certification/repair-resume/hair-components.json',JSON.stringify(results,null,2));console.log(JSON.stringify(results.map(r=>({...r,parts:r.parts.filter(p=>p.bounds.min[1]>1.5).map(({triangles,...p})=>p)})),null,2));
