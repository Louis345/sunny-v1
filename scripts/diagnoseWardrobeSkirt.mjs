...
const source=await readFile(new URL('../web/public/companions/matilda-wardrobe-identity-preserved-v2.vrm',import.meta.url));
...
try {
 for(const mode of ['bottom-material-ids']) {
  ...
  if(mode==='bottom-material-ids')for(const [index,color] of [[9,[1,0,0,1]],[14,[0,1,0,1]],[16,[0,0,1,1]],[17,[1,1,0,1]]]){
   json.materials[index]={name:json.materials[index].name,extras:json.materials[index].extras,pbrMetallicRoughness:{baseColorFactor:color},extensions:{KHR_materials_unlit:{}},doubleSided:true};
   json.extensions.VRM.materialProperties[index].shader='VRM_USE_GLTFSHADER';
  }
  ...
  await page.getByRole('button',{name:'Preview Comet Hoodie',exact:true}).click();
  await page.getByLabel('Inspection view',{exact:true}).selectOption('front');
