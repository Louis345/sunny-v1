...
function isSunnySubject(value: string): value is SunnySubject {
  return (
    value === "homework" ||
    value === "review" ||
    value === "reading" ||
...
function normalizeSubject(raw: string | undefined): SunnySubject | null {
  ...
  return value && isSunnySubject(value) ? value : null;
}

function normalizeSessionMode(raw: string | undefined): SunnySessionMode | null {
  const value = raw?.trim().toLowerCase();
  if (value === "diag" || value === "as-child" || value === "intro" || value === "real") {
    return value;
  }
  return null;
}
...
function normalizePreviewMode(raw: string | undefined): SunnyPreviewMode | null {
  const value = raw?.trim().toLowerCase();
  if (value === "free" || value === "go-live" || value === "off") return value;
...
function normalizeNodeAccess(raw: string | undefined): SunnyNodeAccess | null {
  const value = raw?.trim().toLowerCase();
  if (value === "inspect-all" || value === "normal") return value;
  return null;
}
