...
export async function runGoLivePreviewHomework(argv: string[]): Promise<void> {
  ...
  if (pendingNodes.length === 0) {
    ...
    for (const [idx, node] of fallback.entries()) {
      ...
      if (!url) continue;
      console.log(`📖 Node ${idx + 1}: quest → opening Chrome (go-live)...`);
      console.log(`   URL: ${url}`);
      await openUrl(url);
      await delay(500);
    }
  ...
  for (const [idx, node] of pendingNodes.entries()) {
    ...
    console.log(`${icon} Node ${n}: ${node.type} → opening Chrome (go-live)...`);
    console.log(`   URL: ${url}`);
    await openUrl(url);
    await delay(500);
  }
...
if (typeof require !== "undefined" && require.main === module) {
  runGoLivePreviewHomework(process.argv.slice(2)).catch((err) => {
    console.error("🎮 [goLivePreview] failed", err);
    process.exit(1);
  });
}
